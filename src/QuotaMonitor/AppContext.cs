...
namespace QuotaMonitor
{
    ...
    {
        /// <summary>用户数据根目录（Documents\QuotaMonitor）。</summary>
        ...
        static AppContext()
        {
            // 旧版本数据目录（DeepSeek余额监控）自动迁移到新目录
            try
            {
                var old = Path.Combine(
                    Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments),
                    "DeepSeek余额监控");
                if (Directory.Exists(old) && !Directory.Exists(DataRoot))
                {
                    Directory.Move(old, DataRoot);
                }
            }
            catch { }
        }
